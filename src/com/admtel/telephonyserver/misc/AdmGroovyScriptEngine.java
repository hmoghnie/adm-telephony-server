 ...
 */
public class AdmGroovyScriptEngine implements ResourceConnector {
                ...
                public LookupResult findClassNode(String origName, CompilationUnit compilationUnit) {
                    ...
                    for (String ext : cc.getScriptExtensions()) {
                        try {
                            ...
                            if (AdmGroovyScriptEngine.this.isSourceNewer(entry)) {
                                try {
        ...
        private Class doParseClass(GroovyCodeSource codeSource) {
            ...
            if (origDep != null) {
                ...
                for (String depName : origDep) {
                    ...
                    try{
                        if (origEntry==dep || AdmGroovyScriptEngine.this.isSourceNewer(dep)) {
                            newDep.add(depName);
                        }
                    } catch (ResourceException re) {
    ...
    public static void main(String[] urls) throws Exception {
        AdmGroovyScriptEngine gse = new AdmGroovyScriptEngine(urls);
     ...
     */
    private AdmGroovyScriptEngine(URL[] roots, ClassLoader parent, ResourceConnector rc) {
    ...
    public AdmGroovyScriptEngine(URL[] roots) {
        this(roots, CL_STUB, null);
    }

    public AdmGroovyScriptEngine(URL[] roots, ClassLoader parentClassLoader) {
        this(roots, parentClassLoader, null);
    }

    public AdmGroovyScriptEngine(String[] urls) throws IOException {
    ...
    public AdmGroovyScriptEngine(String[] urls, ClassLoader parentClassLoader) throws IOException {
        this(createRoots(urls), parentClassLoader, null);
    }

    public AdmGroovyScriptEngine(String url) throws IOException {
        this(new String[]{url});
    }

    public AdmGroovyScriptEngine(String url, ClassLoader parentClassLoader) throws IOException {
        this(new String[]{url}, parentClassLoader);
    }

    public AdmGroovyScriptEngine(ResourceConnector rc) {
        this(null, CL_STUB, rc);
    }

    public AdmGroovyScriptEngine(ResourceConnector rc, ClassLoader parentClassLoader) {
     ...
     * ClassLoader that loaded the <code>AdmGroovyScriptEngine</code> class.
     *
     * @return the parent classloader used to load scripts
     */
